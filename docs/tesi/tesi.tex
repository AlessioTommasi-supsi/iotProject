...
\[ R = \frac{V_{\text{ch0}}}{9\,\si{\micro\ampere}} - 1\,\si{\kilo\ohm} \] 

sfortunatamente i dati letti da esp con questa formula non hanno alcun senso!
vedi la seguente tabella con le misure di tensione di resistenze note: 

\paragraph{Valori Sperimentali}
\begin{tabular}{|l|c|r|}
    \hline
    Resistenza [Ohm]  & Tensione [V] & Note\\
    \hline
    1k & 0.67 & bbb\\
    \hline
    100k & 2.43 & ccc\\
    \hline
    inf & 2.5 & nessuna resistenza collegata\\
    \hline
\end{tabular}

\subparagraph{Conclusioni}
La funzione razionale scelta per interpolare esattamente i tre punti e garantire l’asintoto \(V(\infty)=2.5\) è:

Da Trovare per ora semplicemente tensione misurata/ 9uA

Nota: per hora ho notato che tensione letta e esattamente quella sulla resistenza esterna. invece che essere una partizione! 
